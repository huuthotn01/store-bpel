...
func (a *accountServiceAdapter) CreateAccount(ctx context.Context, request *schema.AddAccountRequest) error {
	...
	if err != nil {
		log.Printf("Customer Service-AccountServiceAdapter-AddAccount-Marshal error %v", err)
		return err
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, url, bytes.NewReader(data))
	if err != nil {
		log.Printf("Customer Service-AccountServiceAdapter-AddAccount-NewRequestWithContext error %v", err)
		return err
	}
	...
	if err != nil {
		log.Printf("Customer Service-AccountServiceAdapter-AddAccount-httpClient.Do error %v", err)
		return err
	}

	respByteArr, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		log.Printf("Customer Service-AccountServiceAdapter-AddAccount-ioutil.ReadAll error %v", err)
		return err
	}

	err = json.Unmarshal(respByteArr, &result)
	if err != nil {
		log.Printf("Customer Service-AccountServiceAdapter-AddAccount-json.Unmarshal error %v", err)
		return err
	}
