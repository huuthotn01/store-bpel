...
func Test_goodsServiceController_handleExport(t *testing.T) {
	type args struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := c.handleExport(ctx, tt.args.request); (err != nil) != tt.wantErr {
				t.Errorf("handleExport() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
